...
rm -f ./data/quality_time_bitgpu.dat
...
do
	...
	rm -f ./data/raw_bitgpu_$DESIGN.dat
	...
	do
		./bin/bitgpu_driver ./data/$DESIGN/$DESIGN.asm \
			...
			1 \
			1 \
			$BLOCK | grep -v "good" >> ./data/raw_bitgpu_$DESIGN.dat
	done
	cat ./data/raw_bitgpu_$DESIGN.dat | mymin > ./data/quality_time_bitgpu_$DESIGN.dat
	cat ./data/raw_bitgpu_$DESIGN.dat | mymin | sed "s/^/$DESIGN,/" > ./data/quality_time_bitgpu.dat
done
