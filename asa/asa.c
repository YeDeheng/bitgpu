...
#define ASA_ID "/* $Id: asa.c,v 29.6 2013/10/19 21:31:00 ingber Exp ingber $ */"
    ...
    for (sort_index = 0; sort_index < OPTIONS->Gener_Block; ++sort_index) {
      ...
      *accepted_to_generated_ratio =
      ...
      if (current_generated_state->cost < best_generated_state->cost) {
        best_flag = 1;
//	printf("Nachiket says best %d of %d\n",*number_generated,OPTIONS->Limit_Generated);
      } else {
        best_flag = 0;
      }
      ...
      VFOR (index_v) {
      	search_space *= (-parameter_minimum[index_v]+best_generated_state->parameter[index_v]+1);
      }

      if (iter >= OPTIONS->Limit_Generated) // || search_space<=16777216)
      {
//	      printf("Target iterations reached %d out of %d\n",iter, OPTIONS->Limit_Generated);
